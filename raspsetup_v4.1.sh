...
# --- Biến toàn cục ---
TITLE="RaspSetup v4.1 R2"
BACKTITLE="123Labs-96 | RaspSetup v4.1 R2"
...
check_app_installed() {
    ...
    case "$app_name" in
        ...
        "pihole")
            ...
            ;;
        "adguardhome")
            # Kiểm tra nếu thư mục cài đặt tồn tại và thực thi chính có thể chạy
            [ -d "/opt/AdGuardHome" ] && [ -x "/opt/AdGuardHome/AdGuardHome" ]
            ;;
        *)
...
# --- Hàm: Cài đặt ứng dụng qua apt (Tối ưu cho Neofetch, RealVNC, Htop) ---
install_via_apt() {
    local packages="$1"
    local app_name="$2" # Tên để hiển thị
    echo "--- Installing $app_name... ---"
    # Sử dụng gauge để hiển thị tiến độ apt install
    # Cải tiến: Ghi log đầu ra để phân tích lỗi nếu cần
    local log_file="/tmp/raspsetup_apt_install_${app_name// /_}.log"
    if (
        sudo apt-get install -y $packages 2>&1 | \
        stdbuf -o0 awk '/\[/{print $3}' | \
        stdbuf -o0 awk 'BEGIN{progress=0} {progress=progress+1; print int(progress*100/NR) "\n# Installing '$app_name'..."}' | \
        whiptail --gauge "Installing $app_name..." 6 60 0
    ) && [ -f "$log_file" ] && ! grep -q "Err\|E:" "$log_file"; then
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "$app_name has been installed successfully." 10 60
        rm -f "$log_file" # Dọn dẹp log nếu thành công
        return 0
    else
        # Nếu thất bại, đọc log hoặc đầu ra để hiển thị lỗi
        local error_msg
        if [ -f "$log_file" ]; then
            error_msg=$(grep -E "(Err|E:|Failed|Error)" "$log_file" | tail -n 10)
        else
            error_msg="Installation command failed or log file missing."
        fi
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Failed to install $app_name. Error:\n\n$error_msg" 15 70
        rm -f "$log_file" # Dọn dẹp log nếu thất bại
        return 1
    fi
}
...
# --- Hàm: Cài đặt Pi-Hole (TỐI ƯU) ---
install_pihole() {
    echo "--- Installing Pi-Hole... ---"
    local temp_script="$HOME/pihole-install-temp.sh"
    local log_file="$HOME/pihole-install.log"

    # Tải script chính thức
    if ! curl -sSL -o "$temp_script" https://install.pi-hole.net; then
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Failed to download Pi-Hole install script using curl." 10 60
        rm -f "$temp_script"
        return 1
    fi

    # Ghi log đầu ra khi chạy script
    if sudo bash "$temp_script" > "$log_file" 2>&1; then
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Pi-Hole installed successfully!" 10 60
        # Tự động mở cấu hình Pi-Hole sau khi cài đặt
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Opening Pi-Hole configuration menu..." 10 60
        # Chạy pihole -c trong môi trường terminal nếu có thể, nếu không thì thông báo
        if command -v pihole >/dev/null 2>&1; then
            # Gợi ý người dùng mở terminal để tương tác
            whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Pi-Hole is installed. Please open a new terminal and run 'pihole -c' to configure." 10 60
            # Hoặc nếu muốn chạy trong script (có thể gây lỗi nếu không tương tác đúng cách)
            # sudo pihole -c
        else
            whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Pi-Hole installed, but 'pihole' command not found. Please check installation." 10 60
        fi
        rm -f "$temp_script" # Dọn dẹp script sau khi chạy thành công
        rm -f "$log_file" # Dọn dẹp log
        return 0
    else
        # Nếu thất bại, đọc log để hiển thị lỗi
        local error_msg=$(tail -n 20 "$log_file" 2>&1)
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Pi-Hole installation failed. Last 20 lines of log:\n\n$error_msg" 15 70
        rm -f "$temp_script" # Dọn dẹp script sau khi chạy thất bại
        rm -f "$log_file" # Dọn dẹp log
        return 1
    fi
}

# --- Hàm: Cài đặt AdGuard Home (TỐI ƯU) ---
install_adguardhome() {
    echo "--- Installing AdGuard Home... ---"
    local temp_script="$HOME/adguard-install-temp.sh"
    local log_file="$HOME/adguard-install.log"

    # Tải script chính thức
    if ! curl -s -S -L -o "$temp_script" https://raw.githubusercontent.com/AdguardTeam/AdGuardHome/master/scripts/install.sh; then
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Failed to download AdGuard Home install script using curl." 10 60
        rm -f "$temp_script"
        return 1
    fi

    # Ghi log đầu ra khi chạy script
    # Lưu ý: Script AdGuard Home yêu cầu tương tác người dùng, nên cần chạy với quyền root
    # hoặc có thể yêu cầu người dùng nhập mật khẩu sudo trong quá trình chạy.
    if sh "$temp_script" > "$log_file" 2>&1; then
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "AdGuard Home installed successfully!\n\nPlease open your web browser and go to http://<IP_ADDRESS>:3000 to configure." 10 60
        rm -f "$temp_script" # Dọn dẹp script sau khi chạy thành công
        rm -f "$log_file" # Dọn dẹp log
        return 0
    else
        # Nếu thất bại, đọc log để hiển thị lỗi
        local error_msg=$(tail -n 20 "$log_file" 2>&1)
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "AdGuard Home installation failed. Last 20 lines of log:\n\n$error_msg" 15 70
        rm -f "$temp_script" # Dọn dẹp script sau khi chạy thất bại
        rm -f "$log_file" # Dọn dẹp log
        return 1
    fi
            ...
            for choice in $CHOICES; do
                case $choice in
                    ...
                    "\"10\"")
                        ...
                        ;;
                    "\"11\"")
                        if check_app_installed "pihole"; then
                            whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Pi-Hole is already installed." 10 60
                        else
                            install_pihole
                        fi
                        ;;
                    "\"12\"")
                        if check_app_installed "adguardhome"; then
                            whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "AdGuard Home is already installed." 10 60
                        else
                            install_adguardhome
                        fi
                        ;;
