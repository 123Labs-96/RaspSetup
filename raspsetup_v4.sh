...
# --- Biến toàn cục ---
TITLE="RaspSetup v4.0"
BACKTITLE="Orwae Enterprise LLC | RaspSetup v4.0"
...
install_pi_apps() {
    ...
    if ! wget -qO "$temp_script" https://raw.githubusercontent.com/Botspot/pi-apps/master/install; then
        ...
        rm -f "$temp_script" # Dọn dẹp nếu tải thất bại
        return 1
    fi
    ...
    if [ -f "$temp_script" ]; then
        chmod +x "$temp_script"
        # Chạy script dưới quyền người dùng hiện tại (không sudo)
        ...
        rm -f "$temp_script" # Dọn dẹp nếu tệp không tồn tại
        return 1
    fi
}

# --- Hàm: Cài đặt Webmin (SỬA CHO TRIXIE - DÙNG REPO NEWKEY) ---
install_webmin() {
    echo "--- Installing Webmin using 'newkey' repository for Trixie... ---"
    ...
    # 2. Cài đặt phụ thuộc cần thiết
    if ! sudo apt-get install -y wget gnupg; then
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Failed to install Webmin setup dependencies (wget, gnupg). Cannot proceed." 10 60
        return 1
    fi

    # 3. Import khóa GPG mới từ Webmin (dành cho repo newkey)
    # Sử dụng tệp tạm để lưu khóa trước khi import
    local key_file="/tmp/webmin-newkey.gpg"
    if ! curl -fsSL https://download.webmin.com/download/newkey/repository/webmin.asc -o "$key_file"; then
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Failed to download Webmin newkey GPG key. Cannot proceed." 10 60
        rm -f "$key_file"
        return 1
    fi
    if ! (sudo gpg --dearmor -o /usr/share/keyrings/webmin-newkey-archive-keyring.gpg < "$key_file"); then
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Failed to import Webmin newkey GPG key. Cannot proceed." 10 60
        rm -f "$key_file"
        return 1
    fi
    rm -f "$key_file" # Dọn dẹp tệp tạm

    # 4. Thêm repo 'newkey' (stable) vào apt sources.list.d
    # Repo này nên tương thích tốt hơn với Trixie
    local repo_file="/etc/apt/sources.list.d/webmin-stable.list"
    if ! echo 'deb [signed-by=/usr/share/keyrings/webmin-newkey-archive-keyring.gpg] https://download.webmin.com/download/newkey/repository stable main' | sudo tee "$repo_file" > /dev/null; then
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Failed to add Webmin newkey repository. Cannot proceed." 10 60
        return 1
    fi

    # 5. Cập nhật lại apt sau khi thêm repo mới
    if ! update_apt; then
        whiptail --title "$TITLE" --backtitle "$BACKTITLE" --msgbox "Failed to update package lists after adding Webmin repository. Cannot proceed." 10 60
        return 1
    fi
